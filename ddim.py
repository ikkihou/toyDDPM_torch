...
def get_selection_schedule(schedule, size, timesteps):
    ...
    if schedule == "linear":
        ...
        subsequence = (
            torch.pow(torch.linspace(0, math.sqrt(timesteps * 0.8), size), 2)
            .round()
            .to(torch.int64)
        )  # noqa
...
class DDIM(ddpm_torch.GaussianDiffusion):
    def __init__(
        self, betas, model_mean_type, model_var_type, loss_type, eta, subsequence
    ):
        ...
        try:
            assert not (
                eta2 != 1.0 and model_var_type != "fixed-small"
        ...
        self.posterior_var = (
            self.betas * (1.0 - self.alphas_bar_prev) / (1.0 - self.alphas_bar) * eta2
        )
        self.posterior_logvar_clipped = torch.log(
            torch.cat([self.posterior_var[[1]], self.posterior_var[1:]]).clip(min=1e-20)
        )

        # coefficients to recover x_0 from x_t and \epsilon_t
        self.sqrt_recip_alphas_bar = torch.sqrt(1.0 / self.alphas_bar)
        self.sqrt_recip_m1_alphas_bar = torch.sqrt(1.0 / self.alphas_bar - 1.0)

        # coefficients to calculate E[x_{t-1}|x_0, x_t]
        self.posterior_mean_coef2 = (
            torch.sqrt(1 - self.alphas_bar - eta2 * self.betas)
            * torch.sqrt(1 - self.alphas_bar_prev)
            / (1.0 - self.alphas_bar)
        )
        self.posterior_mean_coef1 = self.sqrt_alphas_bar_prev * (
            1.0 - torch.sqrt(self.alphas) * self.posterior_mean_coef2
        )
    ...
    def p_sample(
        self, denoise_fn, shape, device=torch.device("cpu"), noise=None, seed=None
    ):
...
if __name__ == "__main__":
    ...
    print(DDIM.from_ddpm(diffusion, eta=0.0, subsequence=subsequence).__dict__)

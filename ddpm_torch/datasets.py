...
def crop_celeba(img):
    return transforms.functional.crop(
        img, top=40, left=15, height=148, width=148
    )  # noqa
...
class CelebA(tvds.VisionDataset):
    ...
    _transform = transforms.Compose(
        [crop_celeba, transforms.Resize((64, 64)), transforms.PILToTensor()]
    )
    all_size = 202599
    train_size = 162770
    val_size = 19867
...
class CelebA_HQ(tvds.VisionDataset):
    ...
    def __init__(self, root, split, transform=None):  # unused
        super().__init__(root, transform=transform or self._transform)
        self.filename = sorted(
            [
                fname
                for fname in os.listdir(
                    os.path.join(root, self.base_folder, "img_celeba_hq")
                )
...
def get_dataloader(
    ...
    **kwargs
    ...
    if split != "test" and val_size > 0.0:
        if hasattr(dataset_info, "train_size") and not hasattr(
    ...
    dataloader_configs["sampler"] = sampler = (
        DistributedSampler(dataset, shuffle=True, seed=random_seed, drop_last=drop_last)
        if distributed
        else None
    )
    dataloader_configs["shuffle"] = (
        (sampler is None) if split in {"train", "all"} else False
    ) and not raw

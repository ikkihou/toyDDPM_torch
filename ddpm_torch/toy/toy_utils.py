...
class Evaluator:
    ...
    def eval(self, sample_fn):
        ...
        gen_hist = 0
        for _ in range(
            0, self.max_eval_count + self.eval_batch_size, self.eval_batch_size
        ):
            x_gen.append(sample_fn(self.eval_batch_size))
            gen_hist += hist2d(x_gen[-1], bins=self.bins, value_range=self.value_range)

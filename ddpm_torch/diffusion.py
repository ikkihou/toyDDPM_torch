...
def get_beta_schedule(
    beta_schedule, beta_start, beta_end, timesteps, dtype=torch.float64
):
    if beta_schedule == "quad":
        betas = (
            torch.linspace(beta_start**0.5, beta_end**0.5, timesteps, dtype=dtype) ** 2
        )
...
class GaussianDiffusion:
    ...
    def __init__(self, betas, model_mean_type, model_var_type, loss_type, **kwargs):
        ...
        self.sqrt_recip_m1_alphas_bar = torch.sqrt(
            1.0 / self.alphas_bar - 1.0
        )  # m1: minus 1
        ...
        self.posterior_mean_coef1 = (
            betas * sqrt_alphas_bar_prev / (1.0 - self.alphas_bar)
        )
        self.posterior_mean_coef2 = (
            torch.sqrt(alphas) * (1.0 - alphas_bar_prev) / (1.0 - self.alphas_bar)
        )
    ...
    def q_mean_var(self, x_0, t):
        ...
        var = self._extract(1.0 - self.alphas_bar, t, x_0)
    ...
    def p_mean_var(self, denoise_fn, x_t, t, clip_denoised, return_pred):
        ...
        if self.model_var_type == "learned":
            ...
            model_var, model_logvar = self._extract(
                self.fixed_model_var, t, x_t
            ), self._extract(self.fixed_model_logvar, t, x_t)
        else:
            raise NotImplementedError(self.model_var_type)

        # calculate the mean estimate
        _clip = (lambda x: x.clamp(-1.0, 1.0)) if clip_denoised else (lambda x: x)
    ...
    def p_sample_step(
        self, denoise_fn, x_t, t, clip_denoised=True, return_pred=False, generator=None
    ):
    ...
    def p_sample(
        self, denoise_fn, shape=None, device=torch.device("cpu"), noise=None, seed=None
    ):
    ...
    def _loss_term_bpd(self, denoise_fn, x_0, x_t, t, clip_denoised, return_pred):
        ...
        kl = flat_mean(kl) / math.log(2.0)  # natural base to base 2
        decoder_nll = discretized_gaussian_loglik(
            x_0, model_mean, log_scale=0.5 * model_logvar
        ).neg()
    ...
    def _prior_bpd(self, x_0):
        ...
        kl_prior = normal_kl(T_mean, T_logvar, mean2=0.0, logvar2=0.0)
        return flat_mean(kl_prior) / math.log(2.0)

...
def train(rank=0, args=None, temp_dir=""):
    ...
    out_channels = (
        2 * in_channels if diffusion_config.model_var_type == "learned" else in_channels
    )
    ...
    if distributed:
        ...
        if args.rigid_launch:
            ...
            dist.init_process_group(
                "nccl", init_method=init_method, rank=rank, world_size=args.num_gpus
            )
            local_rank = rank
    ...
    if args.use_ddim:
        subsequence = get_selection_schedule(
            args.skip_schedule,
            size=args.subseq_size,
            timesteps=diffusion_config.timesteps,
        )
        diffusion_eval = DDIM.from_ddpm(diffusion, eta=0.0, subsequence=subsequence)
    else:
    ...
    if resume:
        try:
            map_location = (
                {"cuda:0": f"cuda:{local_rank}"} if distributed else train_device
            )
...
def main():
    ...
    parser.add_argument(
        "--exp-name", type=str, help="name of the current experiment run"
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="number of mini-batches before an update",
    )
    parser.add_argument(
        "--block-size", default=1, type=int, help="block size used for pixel shuffle"
    )
    parser.add_argument(
        "--timesteps", default=1000, type=int, help="number of diffusion steps"
    )
    parser.add_argument(
        "--beta-schedule",
        choices=[
    ...
    parser.add_argument(
        "--model-mean-type", choices=["mean", "x_0", "eps"], default="eps", type=str
    )
    parser.add_argument(
        "--model-var-type",
    ...
    parser.add_argument(
        ...
        default=64,
        type=int,
        help="number of images to sample and save",
    )
    ...
    parser.add_argument(
        "--resume", action="store_true", help="to resume training from a checkpoint"
    )
    parser.add_argument(
        "--chkpt-path",
        default="",
    ...
    parser.add_argument(
        "--eval", action="store_true", help="whether to evaluate fid during training"
    )
    ...
    parser.add_argument(
        "--use-ddim",
        action="store_true",
        help="whether to use DDIM sampler for evaluation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ema-decay", default=0.9999, type=float, help="decay factor of ema"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rigid-launch",
        action="store_true",
        help="whether to use torch multiprocessing spawn",
    )
    parser.add_argument(
        "--num-gpus",
        default=1,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="test-run till the first model update completes",
    )

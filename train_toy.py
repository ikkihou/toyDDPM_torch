...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset",
        choices=["gaussian8", "gaussian25", "swissroll"],
        default="gaussian8",
    )
    ...
    parser.add_argument(
        "--lr-warmup", default=0, type=int, help="number of warming-up epochs"
    )
    parser.add_argument("--batch-size", default=1000, type=int)
    parser.add_argument(
        "--timesteps", default=100, type=int, help="number of diffusion steps"
    )
    parser.add_argument(
        "--beta-schedule",
    ...
    scheduler = (
        lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda t: min((t + 1) / warmup, 1.0))
        if warmup > 0
        else None
    )
    ...
    trainer.train(
        evaluator, chkpt_path=chkpt_path, image_dir=image_dir, xlim=xlim, ylim=ylim
    )
